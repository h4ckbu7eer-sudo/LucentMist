...
public class GatewayConvergenceTests
{
    ...
    public void FailedDiscoveryRemainsVisibleWithoutDemandingEndlessRetry()
    {
        ...
        Assert.NotEmpty(SecurityAnalysisEvidence.FindIncompleteChecks("分析 192.168.2.1 安全", observations));
        Assert.Empty(SecurityAnalysisEvidence.FindIncompleteChecks("分析 192.168.2.1 安全", observations, includeFailedAttempts: false));
        Assert.Contains("网络不可达", SecurityAnalysisEvidence.LimitedAssessment("分析 192.168.2.1 安全", observations));
    }
}
