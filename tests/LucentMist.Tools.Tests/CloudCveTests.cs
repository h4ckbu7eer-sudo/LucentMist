...
public class CloudCveTests
{
    ...
    public async Task Default_QueriesThreeRealContracts_WithoutTargetOrBannerLeak()
    {
        ...
        var report = await CveApiClient.QueryWithStatusAsync("SSH-2.0-OpenSSH_9.8p1 host=192.168.2.1 token=TEST_SECRET", 22,
            http, CveApiClient.IsExternalEnabled(null), CancellationToken.None);
        Assert.Equal(3, requests.Count);
        Assert.All(requests, url => { Assert.DoesNotContain("192.168.2.1", url); Assert.DoesNotContain("TEST_SECRET", url); });
