...
- 最终轮经 2026-09-01 重复键勘误为 **16/17（94.12%）**，仍达到至少 10 次、≥90% 的门槛，但不是旧统计所称的 17/17。[勘误与复算方式](upstream-device-remediation.md)保留原始失败回复。
- 发现后实际修复：VMware 厂商误映射、证书自签幻觉/错误处置建议、空最终答案、结论检查误拦截。每次生产修复后重跑全量测试和真实对话，最终 **533 个 .NET 测试通过，0 警告、0 错误**；另有 14 个 Python 检查器测试通过。
- 网关独立问题的初始错误总结被引擎拒绝并要求修正；最终交付总结保留 DNS 探测差异、TLS 信任风险及未知版本边界。结构契约达标也不等于“原始模型语义从不出错”。
