...
```powershell
dotnet run --project src/LucentMist.CLI -c Release --no-build -- vuln-scan 192.168.2.1 --authorized
dotnet run --project src/LucentMist.CLI -c Release --no-build -- ssl-check 192.168.2.1 --port 443 --authorized
```
...
dotnet run --project src/LucentMist.CLI -- agent "分析 192.168.2.1 的安全风险"
```
