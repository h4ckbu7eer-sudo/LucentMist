...
首次统一策略后的远端 hourly（run `33358190504`）所有 364 个 Tools 测试通过，但 `BoundedDispatcher_FastTcpIsNotBlockedBySlowPing` 因快任务耗时 374ms 超过硬编码 250ms 失败；慢任务耗时 504ms、峰值并发 2，日志并不支持“快任务被串行阻塞”的判断。

该测试改用同步信号：慢任务明确开始并阻塞在释放信号；断言快任务已完成而慢任务仍未放行，同时峰值并发为 2，最后释放慢任务并回收 dispatcher。15 秒只用于终止死锁，不是性能达标阈值。串行实现无法在慢任务放行前完成快任务，因此仍会失败。真实延迟继续记录，不因宿主机调度变慢就判逻辑错误；没有改动生产调度器。
