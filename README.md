...
手册已对齐 0.9.8；旧发布镜像不会自动获得新修复，升级与回滚请按发布说明核对版本和摘要。
...
[![Tests](https://img.shields.io/badge/local-tests-590%2F590%20passed-brightgreen)]()
...
docker build --build-arg NUGET_SOURCE=https://repo.huaweicloud.com/repository/nuget/v3/index.json -t lucentmist:0.9.8 .

# 私有 GHCR 包先登录；令牌需要 read:packages，且不要写入脚本
echo "$CR_PAT" | docker login ghcr.io -u YOUR_GITHUB_USER --password-stdin

# 拉取新版本；需要不可变部署时使用 RELEASE_0.9.8.md 中核实后的 manifest digest
docker pull ghcr.io/h4ckbu7eer-sudo/lucentmist:0.9.8
docker tag ghcr.io/h4ckbu7eer-sudo/lucentmist:0.9.8 lucentmist:0.9.8
...
| [0.9.8 发布说明](docs/RELEASE_0.9.8.md) | 核心变更、验证证据与部署要求 |
