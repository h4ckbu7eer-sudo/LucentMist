    ...
    internal static string ForModel(ReActObservation observation)
    {
        try
        {
            ...
            if (root is JsonObject evidence && evidence["dnsSecurity"] is JsonObject dns)
                dns["interpretation"] = "必须按 versionAssessment/queries 描述：无有效响应只能说版本未知、未取得响应，不能说版本被隐藏或未公开；有响应但无版本值才说本次响应未提供版本。amplificationRatio 只是一次响应/请求字节比，不是风险等级，不能据此称低风险。公网可达性/反射能力未验证。无需为这些未知项反复补查。";
